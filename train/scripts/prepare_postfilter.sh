...
mkdir -p $expdir/acosutic/norm/
python $NNSVS_COMMON_ROOT/extract_static_scaler.py \
    $dump_norm_dir/out_acoustic_scaler.joblib \
    $expdir/acosutic/model.yaml \
    $dump_norm_dir/out_postfilter_scaler.joblib

for s in ${datasets[@]};
do
    if [ ! -e $expdir/acosutic/${acoustic_eval_checkpoint} ]; then
        echo "ERROR: acoustic model checkpoint $expdir/acosutic/${acoustic_eval_checkpoint} does not exist."
    ...
    xrun nnsvs-gen-static-features \
        model.checkpoint=$expdir/acosutic/${acoustic_eval_checkpoint} \
        model.model_yaml=$expdir/acosutic/model.yaml \
        out_scaler_path=$dump_norm_dir/out_acoustic_scaler.joblib \
        in_dir=$dump_norm_dir/$s/in_acoustic/ \
        out_dir=$expdir/acosutic/norm/$s/in_postfilter \
...
# for convenience
cp -v $dump_norm_dir/out_postfilter_scaler.joblib $expdir/acosutic/norm/in_postfilter_scaler.joblib

# apply normalization for input features
# NOTE: output features are already normalized
# find $expdir/acosutic/org/$train_set/in_postfilter -name "*feats.npy" > train_list.txt
# scaler_path=$expdir/acosutic/norm/in_postfilter_scaler.joblib
...
#     xrun nnsvs-preprocess-normalize in_dir=$expdir/acosutic/org/$s/in_postfilter \
